...
import { confirmarPedido, crearPedido, listarPedidosRango } from '@/api/operacionesApi'
...
export default function PedidosPage() {
  ...
  const { data: pedidos = [] } = useQuery({ queryKey: ['pedidos', idZona, fechaIni, fechaFin], queryFn: () => listarPedidosRango(idZona, fechaIni, fechaFin) })
  ...
  const crearMutation = useMutation({
    ...
    onSuccess: () => {
      setItems([])
      setObservacion('')
      queryClient.invalidateQueries({ queryKey: ['pedidos', idZona, fechaIni, fechaFin] })
      notifySuccess('Pedido guardado correctamente.')
    },
    onError: (error) => notifyError(getApiErrorMessage(error, 'No se pudo guardar el pedido.')),
  })

  const confirmarMutation = useMutation({
    mutationFn: confirmarPedido,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['pedidos', idZona, fechaIni, fechaFin] })
      notifySuccess('Pedido modificado correctamente.')
    },
    onError: (error) => notifyError(getApiErrorMessage(error, 'No se pudo confirmar el pedido.')),
  })
    ...
    <main className="p-6 space-y-6">
      ...
      <section className="grid gap-6 lg:grid-cols-2">
        <div className="space-y-3">
          ...
          <Button onClick={() => crearMutation.mutate({ observacion, items })} disabled={items.length === 0}>Registrar pedido</Button>
        </div>
      ...
      <section className="grid gap-2 md:grid-cols-3 items-end">
      ...
      <div className="rounded-md border bg-white">
        <Table>
          <TableHeader><TableRow><TableHead>ID</TableHead><TableHead>Fecha</TableHead><TableHead>Estado</TableHead><TableHead>Items</TableHead><TableHead></TableHead></TableRow></TableHeader>
          <TableBody>
            {pedidos.map((p) => (
              <TableRow key={p.idPedido}>
                <TableCell>{p.idPedido}</TableCell>
                <TableCell>{p.fechaRegistro?.slice(0, 19).replace('T', ' ')}</TableCell>
                <TableCell>{p.estadoPedido}</TableCell>
                <TableCell>{p.items?.length ?? 0}</TableCell>
                <TableCell>{p.idPedido && p.estadoPedido === 'PENDIENTE' ? <Button size="sm" onClick={() => confirmarMutation.mutate(p.idPedido!)}>Confirmar</Button> : null}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
